...
def show_page_facturacion(allowed_clients=None):
    ...
    total_neto = facturacion['neto_numerico'].sum().round(0).astype(int)
    importe_total = facturacion['total_numerico'].sum().round(0).astype(int)
    total_saldo = saldos['saldo_numerico'].sum().round(0).astype(int)
