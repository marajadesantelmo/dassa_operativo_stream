...
def fetch_data_orden_del_dia():
    ...
    arribos = arribos[arribos['Cliente'].str.contains('Lawter')].drop(columns=['Cliente'])
    pendiente_desconsolidar = pd.read_csv('data/pendiente_desconsolidar.csv')
    pendiente_desconsolidar = pendiente_desconsolidar[pendiente_desconsolidar['Cliente'].str.contains('Lawter')].drop(columns=['Cliente'])
    verificaciones_impo = pd.read_csv('data/verificaciones_impo.csv')
    verificaciones_impo = verificaciones_impo[verificaciones_impo['Cliente'].str.contains('Lawter')].drop(columns=['Cliente'])
    retiros_impo = pd.read_csv('data/retiros_impo.csv')
    retiros_impo = retiros_impo[retiros_impo['Cliente'].str.contains('Lawter')].drop(columns=['Cliente'])
    otros_impo = pd.read_csv('data/otros_impo.csv')
    otros_impo = otros_impo[otros_impo['Cliente'].str.contains('Lawter')].drop(columns=['Cliente'])
    arribos_expo_carga = pd.read_csv('data/arribos_expo_carga.csv')
    arribos_expo_carga = arribos_expo_carga[arribos_expo_carga['Cliente'].str.contains('Lawter')].drop(columns=['Cliente'])
    arribos_expo_ctns = pd.read_csv('data/arribos_expo_ctns.csv')
    arribos_expo_ctns = arribos_expo_ctns[arribos_expo_ctns['Cliente'].str.contains('Lawter')].drop(columns=['Cliente'])
    verificaciones_expo = pd.read_csv('data/verificaciones_expo.csv')
    verificaciones_expo = verificaciones_expo[verificaciones_expo['Cliente'].str.contains('Lawter')].drop(columns=['Cliente'])
    otros_expo = pd.read_csv('data/otros_expo.csv')
    otros_expo = otros_expo[otros_expo['Cliente'].str.contains('Lawter')].drop(columns=['Cliente'])
    remisiones = pd.read_csv('data/remisiones.csv')
    remisiones = remisiones[remisiones['Cliente'].str.contains('Lawter')].drop(columns=['Cliente'])
    consolidados = pd.read_csv('data/consolidados.csv')
    consolidados = consolidados[consolidados['Cliente'].str.contains('Lawter')].drop(columns=['Cliente'])
