...
def send_email_retiro(row, mail, url=None):
    current_time = datetime.now().strftime('%H:%M')
    if url:
        email_content = f"""
        <html>
        <body>
            <h2>Notificación de Retiro de mercadería de Importación</h2>
            <p>Estimado cliente,</p>
            <p>Le informamos que se realizó un retiro relacionado al contenedor <strong>{row['Contenedor']}</strong> del cliente <strong>{row['Cliente']}</strong> a las <strong>{current_time}</strong>.</p>
            <p>Para más información, por favor visite nuestro <a href="{url}">eTally</a></p>
            <p>Saludos cordiales,</p>
            <p><strong>Alertas automáticas - Dassa Operativo</strong></p>
            <img src="https://dassa.com.ar/wp-content/uploads/elementor/thumbs/DASSA-LOGO-3.0-2024-PNG-TRANSPARENTE-qrm2px9hpjbdymy2y0xddhecbpvpa9htf30ikzgxds.png" alt="Dassa Logo" width="200">
        </body>
        ...
        <body>
            <h2>Notificación de Retiro de mercadería de Importación</h2>
            <p>Estimado cliente,</p>
            <p>Le informamos que se realizó un retiro relacioando al contenedor <strong>{row['Contenedor']}</strong> del cliente <strong>{row['Cliente']}</strong> a las <strong>{current_time}</strong>.</p>
            <p>Saludos cordiales,</p>
            <p><strong>Alertas automáticas - Dassa Operativo</strong></p>
            <img src="https://dassa.com.ar/wp-content/uploads/elementor/thumbs/DASSA-LOGO-3.0-2024-PNG-TRANSPARENTE-qrm2px9hpjbdymy2y0xddhecbpvpa9htf30ikzgxds.png" alt="Dassa Logo" width="200">
        </body>
        ...
        """
    msg = MIMEMultipart()
    msg['Subject'] = 'Notificación Retiro Mercadería de Importación'
    msg['From'] = "auto@dassa.com.ar"
...
for index in range(len(alertas_retiros)):
    ...
    idcont = tally_bi[tally_bi['cont'] == contenedor]['idcont']
    if len(idcont) == 0:
        log(f'No se encontró el contenedor {contenedor} en el sistema eTally')
        print(f'No se encontró el contenedor {contenedor} en el sistema eTally')
        url=None
    else: 
        url = 'http://dassa.mqbic.com/PublicTally.php?id=' + idcont
    if len(client_emails) > 0:
        mails = client_emails[0]
        for mail in mails:
            try:
                send_email_retiro(row, mail, url)
                log(f'Enviado a {mail} CTN {row["Contenedor"]} - Retiro IMPO')
                print(f'Correo enviado a {mail} para el contenedor {row["Contenedor"]}')
            except Exception as e:
                log(f'Error al enviar correo Retiro IMPO a {mail} para el contenedor {row["Contenedor"]}: {e}')
                print(f'Error al enviar correo Retiro IMPO a {mail} para el contenedor {row["Contenedor"]}: {e}')
